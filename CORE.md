...
| Chặn edit | permissions deny `Edit`/`Write` | root: `PreToolUse` hook; peer: read-only sandbox + hook | `agent.<name>.permission.edit: deny` |
...
| Instruction root | `--append-system-prompt` | wrapper set `developer_instructions` | `agent.herdr-orchestrator.prompt` trong `OPENCODE_CONFIG_CONTENT` (runtime-built, thắng project config) |
| Tắt sub-agent | deny `Task`/`Agent` | `[features] multi_agent = false` + hook | `agent.<name>.permission.task: deny` (subagent_depth không tồn tại trong 1.17.20) |
